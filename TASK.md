...
- [In Progress] **Task 6.3: Implement Live LLM Integration Tests** `(Started 2025-04-12)`
  ...
  *Comments:* Exclude from default test runs. Focus on basic API call success and response structure. Added `live` marker to `agentkit/pyproject.toml`. Updated `tox.ini` to exclude `live` tests by default. Created `agentkit/tests/integration/test_live_llm_clients.py`. Fixed issues in test helper. **Debugging Google Client (2025-04-12):** Attempted multiple fixes for `GoogleClient` parameter passing (`config` vs direct kwargs, async vs sync via `asyncio.to_thread`, different models). Consistently failed with contradictory errors (`TypeError: unexpected keyword argument 'temperature'`, `AttributeError: 'GenerationConfig' object has no attribute 'automatic_function_calling'`, `TypeError: unexpected keyword argument 'tools'`). Suggests potential SDK bug/inconsistency in async handling. **Current State:** 3 tests pass (OpenAI, Anthropic, OpenRouter), 1 fails (Google). Google client code uses `asyncio.to_thread` workaround. Test uses `gemini-2.5-pro-exp-03-25`. Next step is further investigation or alternative approach for Google client.
